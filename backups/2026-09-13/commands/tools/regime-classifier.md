...
After running, open all three HTML charts:
```bash
open /Users/macproajb/claude_projects/regime_global_score_{YYYY-MM}.html \
     /Users/macproajb/claude_projects/regime_fingerprint_{YYYY-MM}.html \
     /Users/macproajb/claude_projects/regime_heatmap_{YYYY-MM}.html
```

- **regime_global_score**: Euclidean distance over time — regime (blue) and anti-regime (red) bands, NBER recessions, momentum mask
- **regime_fingerprint**: Horizontal bar chart of current z-scores sorted max→min; blue=positive, red=negative; grouped by vertical category label
- **regime_heatmap**: Full history heatmap (rows=variables, cols=months); thin indicator strip at top shows regime (blue) and anti-regime (red) months aligned to cell grid
