"""
US Leading Boom Indicators Dashboard
Mirror of generate_leading_indicators.py with reference points inverted:
  - Recession dashboard: series vs 24-month PEAK (drawdowns)
  - Boom dashboard: series vs 24-month TROUGH (gains from lows)

Uses fredapi — set FRED_API_KEY env var.
Output: reports/boom-indicators/index.html
"""
...
OUTPUT_PATH  = "reports/boom-indicators/index.html"
...
def plot_series(fig, row, col, series, recession, y0, y1,
                color='#1f77b4', quantile_line=0.8, y_label=''):
...
def create_dashboard():
    ...
    fig = make_subplots(
        ...
        subplot_titles=[
            ...
            "Construction + Manufacturing Jobs: Gains from 24-Month Low",
            "Residential Fixed Investment vs Prior 12Q Low",
        ],
        vertical_spacing=0.08,
        horizontal_spacing=0.08,
    )

    # ── Chart 1: Payrolls Hiring Breadth ─────────────────────────────────────
    print("Chart 1: Payrolls Hiring Breadth...")
    payroll_ids = [
    ...
    plot_series(fig, 1, 1, diffusion1, recession, 0, 100,
                quantile_line=0.8, y_label='% of Industries Rising')
    ...
    # ── Chart 2: EPOP Gains from 24-Month Low ────────────────────────────────
    print("Chart 2: EPOP Gains from 24-Month Low...")
    epop = get_fred("EMRATIO", years=100)
    epop_rel = (epop - epop.rolling(24).min()).dropna()
    epop25 = get_fred("LNS12300060", years=100)
    epop25_rel = (epop25 - epop25.rolling(24).min()).dropna()
    plot_series(fig, 1, 2, epop_rel, recession, 0, 4,
                quantile_line=0.8, y_label='pp above 24M Low')
    fig.add_trace(go.Scatter(
    ...
    # ── Chart 3: Continuing Claims % Below 3-Year High ───────────────────────
    print("Chart 3: Continuing Claims % Below 3-Year High...")
    cc = get_fred("CCSA", years=100)
    cc_rel = ((cc.rolling(156).max() - cc) / cc.rolling(156).max() * 100).dropna()
    plot_series(fig, 2, 1, cc_rel, recession, 0, 100,
                quantile_line=0.8, y_label='% Below 3Y High')

    # ── Chart 4: New Orders Surge from 24-Month Low ───────────────────────────
    print("Chart 4: New Orders Surge from 24-Month Low...")
    no = get_fred("NEWORDER", years=75)
    no_rel = (no / no.rolling(24).min()).dropna()
    ...
    ratio_rel = (ratio / ratio.rolling(24).min()).dropna()
    plot_series(fig, 3, 2, ratio_rel, recession, 1.0, 1.35,
                quantile_line=0.8, y_label='Ratio vs 24M Low')
    ...
    plot_series(fig, 4, 1, diffusion7, recession, 0, 100,
                quantile_line=0.8, y_label='% of Series with Positive YoY')
    ...
    # ── Chart 11: Construction + Mfg Jobs: Gains from 24-Month Low ───────────
    print("Chart 11: Construction + Mfg Jobs: Gains from 24-Month Low...")
    cons = get_fred("USCONS", years=100)
    mfg11 = get_fred("MANEMP", years=100)
    combined11 = (cons + mfg11).dropna()
    trough_24m11 = combined11.rolling(24).min()
    pct_from_trough11 = ((combined11 / trough_24m11) - 1) * 100
    pct_from_trough11 = pct_from_trough11.dropna()

    fig.add_trace(go.Scatter(
        x=pct_from_trough11.index,
        y=pct_from_trough11.clip(lower=0).values,
        fill='tozeroy', fillcolor='rgba(31,119,180,0.25)',
        line=dict(width=0), showlegend=False, hoverinfo='skip',
    ), row=6, col=1)
    fig.add_trace(go.Scatter(
        x=pct_from_trough11.index, y=pct_from_trough11.values,
        mode='lines', showlegend=False,
        line=dict(color='#1f77b4', width=1.8),
        hovertemplate='%{x|%b %Y}: %{y:.2f}%<extra></extra>',
    ), row=6, col=1)
    fig.add_hline(y=0, line_color='black', line_width=1, row=6, col=1)
    add_recession_shading(fig, recession, 6, 1, 0, 25)
    fig.update_yaxes(range=[0, 25], title_text='% Above 24M Low',
                     title_font=dict(size=11), row=6, col=1)
    fig.update_xaxes(title_text='Date', title_font=dict(size=11), row=6, col=1)

    # ── Chart 12: Residential Fixed Investment vs Prior 12Q Low ──────────────
    print("Chart 12: Residential Fixed Investment vs Prior 12Q Low...")
    rfi_raw = get_fred("A011RE1Q156NBEA", years=100)
    prior_12q_min = rfi_raw.rolling(12).min().shift(1)
    rfi_rel = (rfi_raw - prior_12q_min).dropna()
    plot_series(fig, 6, 2, rfi_rel, recession,
                0, rfi_rel.max() * 1.1,
                color='#1f77b4', quantile_line=None,
                y_label='Actual − Prior 12Q Low (Bil. $)')

    # ── Layout ────────────────────────────────────────────────────────────────
    update_time = datetime.now().strftime('%Y-%m-%d %H:%M UTC')
    fig.update_layout(
        title=dict(
            text="US Leading Boom Indicators",
    ...
    fig.add_annotation(
        text=(f'Last Updated: {update_time} | '
              ...
              'Gray shading = NBER recessions (boom indicators should be HIGH in unshaded expansions)'),
...
def main():
    print("=" * 60)
    print("US Leading Boom Indicators Dashboard")
    print("=" * 60)
