...
def transform_vars(sv: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in chg.columns:
        mu  = chg[col].rolling(LOOKBACK_MONTHS, min_periods=60).mean()
        std = chg[col].rolling(LOOKBACK_MONTHS, min_periods=60).std()
        z[col] = ((chg[col] - mu) / std).clip(-3, 3)
...
# ---------------------------------------------------------------------------
# Fingerprint chart — current z-score snapshot
# ---------------------------------------------------------------------------

def plot_fingerprint(
    z_sel: pd.DataFrame, ref_date: pd.Timestamp,
    kept: list[str], out_dir: Path,
) -> Path:
    ref_row = z_sel.loc[ref_date] if ref_date in z_sel.index else z_sel.iloc[-1]
    ...
    for sid in kept:
        ...
        if np.isnan(val):
            continue
        alpha = min(0.35 + abs(val) * 0.22, 1.0)
        color = (f"rgba(30,100,200,{alpha:.2f})" if val >= 0
                 else f"rgba(200,40,40,{alpha:.2f})")
        items.append((sid, val, f"{v} · {sid}", color))

    items.sort(key=lambda x: -x[1])  # descending: highest z at top
    sids   = [i[0] for i in items]
    vals   = [i[1] for i in items]
    labels = [i[2] for i in items]
    colors = [i[3] for i in items]
    ...
    fig = go.Figure(go.Bar(
        ...
        orientation="h",
        marker_color=colors,
        customdata=[[s] for s in sids],
        hovertemplate="%{customdata[0]}<br>Z: %{x:.2f}<extra></extra>",
    ))

    for xv, lw in [(-3, 1.5), (-2, 1.0), (-1, 0.5), (1, 0.5), (2, 1.0), (3, 1.5)]:
        fig.add_vline(x=xv, line=dict(color="lightgray", width=lw, dash="dot"))
    fig.add_vline(x=0, line=dict(color="black", width=0.8))

    fig.update_layout(
        title=dict(
            text=(f"Macro Fingerprint — {ref_date.strftime('%Y-%m')}<br>"
                  f"<sub>12m change · 10yr rolling z-score · winsorized ±3</sub>"),
            font_size=14,
        ),
        xaxis=dict(range=[-3.3, 3.3], title="Z-score"),
        yaxis=dict(autorange="reversed"),
        height=max(450, len(sids) * 22 + 120),
        template="plotly_white",
        margin=dict(l=220, r=40),
        showlegend=False,
    )

    fname = out_dir / f"regime_fingerprint_{ref_date.strftime('%Y-%m')}.html"
    fig.write_html(str(fname))
    print(f"Fingerprint chart: {fname}")
    return fname


# ---------------------------------------------------------------------------
# Z-score heatmap — full history
# ---------------------------------------------------------------------------

def plot_zscore_heatmap(
    z_sel: pd.DataFrame, similar: pd.DatetimeIndex, anti: pd.DatetimeIndex,
    ref_date: pd.Timestamp, kept: list[str], out_dir: Path,
) -> Path:
    _VERT_ORDER = [
        "Output", "Labor", "Housing", "Orders", "Money",
        "Equity", "Rates", "FX", "Prices", "Wages", "Sentiment", "Other",
    ]

    def vsort(s):
        v = _VERTICAL.get(s, "Other")
        return (_VERT_ORDER.index(v) if v in _VERT_ORDER else 99, s)

    sorted_kept = sorted(kept, key=vsort)
    ylabels = [f"{_VERTICAL.get(s, 'Other')} · {s}" for s in sorted_kept]

    mat = z_sel[sorted_kept].T  # (n_vars, n_months)

    # Regime indicator row: -1=similar (blue), +1=anti (red), 0=neutral
    similar_set = set(similar)
    anti_set    = set(anti)
    indicator   = np.array([[
        -1 if dt in similar_set else (1 if dt in anti_set else 0)
        for dt in mat.columns
    ]], dtype=float)

    fig = make_subplots(
        rows=2, cols=1,
        row_heights=[0.045, 0.955],
        shared_xaxes=True,
        vertical_spacing=0.005,
    )

    # Row 1 — regime indicator strip
    fig.add_trace(go.Heatmap(
        x=mat.columns,
        y=["Regime"],
        z=indicator,
        colorscale=[
            [0.0, "rgba(200,40,40,0.75)"],
            [0.5, "rgba(240,240,240,0.2)"],
            [1.0, "rgba(30,100,200,0.75)"],
        ],
        zmin=-1, zmax=1,
        showscale=False,
    ...
    fig.add_trace(go.Heatmap(
        ...
        zmin=-3,
        zmax=3,
        colorbar=dict(title="Z-score", thickness=14, len=0.92, y=0.45),
        hovertemplate="%{y}<br>%{x|%Y-%m}<br>Z: %{z:.2f}<extra></extra>",
    ), row=2, col=1)

    fig.add_vline(
        x=int(ref_date.timestamp() * 1000),
        line=dict(color="orange", width=2, dash="dash"),
        annotation_text=ref_date.strftime("%Y-%m"),
        annotation_position="top right",
    )

    fig.update_layout(
        title=dict(
            text=(f"Z-score Heatmap — {ref_date.strftime('%Y-%m')}<br>"
                  f"<sub>Red=high · Blue=low · Blue strip=regime analog · Red strip=anti-regime</sub>"),
            font_size=14,
        ),
        xaxis2_title="Date",
        yaxis=dict(fixedrange=True),
        yaxis2=dict(autorange="reversed"),
        height=max(540, len(sorted_kept) * 20 + 160),
        template="plotly_white",
        margin=dict(l=220, r=60),
        showlegend=False,
    )

    fname = out_dir / f"regime_heatmap_{ref_date.strftime('%Y-%m')}.html"
    fig.write_html(str(fname))
    print(f"Heatmap chart: {fname}")
    return fname
...
def main():
    ...
    # Trim to rows where at least 80% of kept series are present
    min_cols = max(4, int(len(kept) * 0.80))
    z_sel = z_sel.dropna(thresh=min_cols)
    ...
    plot_fingerprint(z_sel, ref_date, kept, Path(__file__).parent)
    plot_zscore_heatmap(z_sel, similar, anti, ref_date, kept, Path(__file__).parent)
