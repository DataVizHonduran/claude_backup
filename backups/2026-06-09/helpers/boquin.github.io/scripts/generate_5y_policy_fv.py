...
===================================================
Fair value model for the 5Y yield using Fed policy stance variables
as regressors, mirroring the 10Y policy-based fair value model.
...
Model:
  DGS5 = α + β₁·DGS2 + β₂·PolicySpread + β₃·TaylorGap + ε
...
OUTPUT_PATH = os.path.expanduser(
    "~/boquin.github.io/reports/5y-policy-fv/index.html"
)
...
def build_dataset(fred: Fred) -> tuple[pd.DataFrame, object]:
    ...
    dgs5_m = fetch_series(fred, "DGS5",  START_DATE).resample("ME").mean().rename("DGS5")
    ...
    df = pd.concat([dgs5_m, dgs2_m, ffr_m, pce_yoy, unrate, nrou_m], axis=1).dropna()
    ...
    result = sm.OLS(df["DGS5"], X).fit()

    df["FairValue"] = result.fittedvalues
    df["Residual"]  = df["DGS5"] - df["FairValue"]
...
def build_chart(df: pd.DataFrame, result) -> go.Figure:
    fig = make_subplots(
        ...
        subplot_titles=(
            "5-Year Yield vs. Policy-Based Fair Value",
    ...
    fig.add_annotation(
        ...
        text=(
            ...
            f"Actual: <b>{df['DGS5'].iloc[-1]:.2f}%</b><br>"
    ...
    fig.update_layout(
        title=dict(
            text="US 5-Year Treasury: Policy-Based Fair Value Model",
...
<div style="max-width:900px;margin:0 auto 48px;padding:0 16px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;font-size:14px;color:#333;line-height:1.7;">
  ...
  <p>This model estimates the 5-year Treasury yield as a linear function of two Fed policy stance variables, calibrated by OLS regression over the full historical sample.</p>
  ...
  <p style="margin-top:20px;color:#888;font-size:12px;">Sources: Federal Reserve H.15 (DGS5, DGS2, FEDFUNDS), Bureau of Economic Analysis (PCEPILFE), Bureau of Labor Statistics (UNRATE), Congressional Budget Office via FRED (NROU). All data via FRED.</p>
</div>"""
...
<head>
  ...
  <title>US 5Y Treasury: Policy-Based Fair Value</title>
  <style>
...
<body>
  <h1>US 5-Year Treasury: Policy-Based Fair Value Model</h1>
  <p class="subtitle">
